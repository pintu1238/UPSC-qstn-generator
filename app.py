...
class Quiz:
    def __init__(self):
        # intialize empty list to store qstns and user answer and results
        self.questions = []
        self.user_answers = []
        self.results = []

    def  generate_questions(self, generator, topic, question_type, difficulty, num_questions):
        # reset all lists to start fresh
        self.questions = []
        self.user_answers = []
        self.results = []
        ...
        try:
            if question_type.lower() == 'multiple choice':
                mcqs = generator.generate_mcq_set(topic, difficulty.lower(), num_questions)
                for qstn in mcqs:
                    self.questions.append({
                        'type': 'mcq',
                        'question': qstn.question.strip(),
                        'options': qstn.options,
                        'correct_answer': qstn.correct_answer
                    })
            else:
                questions = generator.generate_fill_blank_set(topic, difficulty.lower(), num_questions)
...
def main():
    ...
    if 'quiz_manager' not in st.session_state:
        st.session_state.quiz_manager = Quiz()
